...
import zerocode, packet, template, im as chat_util # local
	...
	client.send(
		...
		zerocode.encode_all(
			...
			packet.pack_sequence(
				...
				packet.rotation,	packet.rotation.zero,	# BodyRotation		16
				packet.rotation,	packet.rotation.zero,	# HeadRotation		16
				packet.u8,			0,						# State				1
				packet.vector,		(128.0, 128.0, 30.0),	# CameraCenter		12
				packet.vector,		(0.0, 1.0, 0.0),		# CameraAtAxis		12
				packet.vector,		(1.0, 0.0, 0.0),		# CameraLeftAxis	12
				packet.vector,		(0.0, 0.0, 1.0),		# CameraUpAxis		12
				packet.f32,			16.0,					# Far				4
				packet.u32,			control if control else 0,				# ControlFlags		4
				packet.u8,			0,						# Flags				1
			)
		)
	)
...
def HandleChatFromSimulator(data: bytes):
	...
	log.info(f'{chat.SourceType} {chat.Type} {chat.Audible} | {chat.FromName}: {chat.Message}')
...
def SendImprovedInstantMessage(text: str):
	...
	client.send(
		packet.header(template.message['ImprovedInstantMessage'], client.sequence, packet.ZEROCODED|packet.RELIABLE),
		zerocode.encode_all(
			chat_util.build_im(
				text,
