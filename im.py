...
def build_im(
		...
		agent_id: bytes,
		session_id: bytes,
		to_agent_id: bytes) -> bytes:
	agent_name_bytes = agent_name.encode('utf-8') + b'\x00'
	text_bytes = text.encode('utf-8') + b'\x00'
	return packet.pack_sequence(
		packet.uuid, agent_id,
		packet.uuid, session_id,
		packet.bool, False,
		packet.uuid, to_agent_id,
		packet.u32, 0,
		packet.uuid, packet.uuid.zero,
		packet.vector, packet.vector.zero,
		packet.u8, 0,
		packet.u8, 0,
		packet.uuid, compute_session_id(Dialog.IM, agent_id, to_agent_id),
		packet.u32, int(time()),
		packet.variable1, len(agent_name_bytes),
		packet.string, agent_name_bytes,
